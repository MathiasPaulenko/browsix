    ...
    async def execute(self, backend: AbstractBackend) -> Any:
        ...
        if action == "get":
            if not self.params.key:
                raise ActionError("key is required for get action")
        ...
        if action == "set":
            if not self.params.key or self.params.value is None:
                raise ActionError("key and value are required for set action")
        ...
        if action == "cache-delete-entry":
            if not self.params.cache_id or not self.params.request_url:
                raise ActionError("cache_id and request_url are required for cache-delete-entry")
        ...
        if action == "cache-cached-response":
            if not self.params.cache_id or not self.params.request_url:
                raise ActionError("cache_id and request_url are required for cache-cached-response")
        ...
        if action == "cache-request-entries":
            if not self.params.cache_id:
                raise ActionError("cache_id is required for cache-request-entries")
        ...
        if action == "indexeddb-get":
            if not self.params.database or not self.params.store:
                raise ActionError("database and store are required for indexeddb-get action")
            return await backend.indexeddb_get_data(self.params.database, self.params.store)

        if action == "indexeddb-clear":
            if not self.params.database or not self.params.store:
                raise ActionError("database and store are required for indexeddb-clear action")
        ...
        if action == "clear-data-for-storage-key":
            if not self.params.storage_key:
                raise ActionError("storage_key is required for clear-data-for-storage-key")
        ...
        if action == "delete-bucket":
            if not self.params.storage_key or not self.params.bucket_name:
                raise ActionError("storage_key and bucket_name are required for delete-bucket")
        ...
        if action == "shared-storage-metadata":
            if not self.params.owner_origin:
                raise ActionError("owner_origin is required for shared-storage-metadata")
        ...
        if action == "set-cookies":
            if self.params.cookies is None:
                raise ActionError("cookies is required for set-cookies")

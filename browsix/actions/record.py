"""Record action: capture browser interactions and generate browsix.yaml."""
...
(function() {
    ...
    const recordEvent = (type, data) => {
        ...
        window.__browsix_record_events = events;
    };
    ...
    console.log('[browsix] Recording started. Interact with the page.');
})();
"""


def events_to_yaml(events: list[dict[str, Any]], initial_url: str) -> str:
    """Convert recorded events to a browsix YAML config.
...
async def record_session(
    ...
    """Record browser interactions and return a browsix YAML config.
    ...
    try:
        raw = await backend.eval(
            "JSON.stringify(window.__browsix_record_events || [])",
            await_promise=True,
        )

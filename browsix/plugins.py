"""Plugin system for browsix — custom actions, backends, and serve middleware.
...
- **actions**: Custom actions for multi-action YAML and serve mode.
  Entry point value must be a ``browsix.plugins.ActionPlugin`` instance.
...
- **hooks**: Classic lifecycle hooks (before_action, after_action, register_cli).
  Entry point value must be a ``browsix.plugins.Plugin`` subclass.
...
ENTRY_POINT_GROUP = "browsix.plugins"
...
class Plugin:
    """Base class for classic browsix lifecycle plugins.
